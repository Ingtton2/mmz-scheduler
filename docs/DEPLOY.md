...
지금은 `https://내앱.vercel.app/admin/...` 주소를 아는 사람은 **누구나** 직원을
추가/삭제할 수 있습니다. 직원 조회용(`/schedule/코드`)만 공개하고 관리자 화면은
막고 싶다면:

1. Render → `mmz-scheduler-api` → Environment 에 추가:
...
이제 관리자 화면에 처음 들어갈 때 브라우저가 아이디/비밀번호를 한 번 묻습니다.
직원 QR 조회 화면은 그대로 열려 있습니다.

> 더 제대로 된 로그인(직원 계정 등)은 나중에 별도 작업으로 추가할 수 있습니다
> (스펙 10.3).
