...
- [x] **4단계 — 연차 정보 자동계산 + 연차 개수 지정 + 사전 휴무 신청** (사전휴무는 사장님이 대신 입력, 기간 단위 시작일~종료일)
      - 연차: 날짜로 신청하지 않음. 사장님이 부여 → 자동배치 화면에서 직원별 "이번 달 사용 개수"를 지정하면 자동배치가 인원이 부족하지 않은 날에 랜덤으로 넣음 (실행 시 잔여연차 차감, 같은 달 재실행은 중복 차감 없음). 근무일수는 그만큼 줄어듦.
...
- [x] **9단계 — 직원 셀프서비스 계정** (`/join` 가입 신청 → 사장님 승인(`/admin/accounts`) → `/staff-login` 이름+PIN 로그인 → `/me` 이하에서 본인 사전휴무 신청(다음 달분만, 20일 마감) + 본인 연차 조회 + 본인 스케줄 조회. 사장님 계정은 즉시 승인, PIN 분실 시 관리자가 초기화)
