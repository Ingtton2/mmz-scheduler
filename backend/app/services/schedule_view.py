...
직원 로그인 후 "이번 달 전체 스케줄"(routes_me.py) 이 이 모양의 데이터를 쓴다.
호출 전에 스케줄이 "confirmed" 상태인지부터 확인해야 한다 (스펙: 임시
상태에서는 직원에게 절대 노출되지 않음). 그리고 당월+다음달 범위 안인지도
함께 확인한다 (그 이후는 공유 상태와 무관하게 항상 차단).
"""
